...
import com.youle.view.SlidingMenu.OnClosedListener;
import com.youle.view.SlidingMenu.OnOpenedListener;
...
/**
 * 这个是最顶层的recipe的容器,效果类似于iweekly
 */
public class CustomViewTop extends ViewGroup {
	...
	public SlidingMenu mCustomViewBehind;
	...
	public CustomViewTop(Context context) {
		this(context, null);
	}

	public CustomViewTop(Context context, AttributeSet attrs) {
		this(context, attrs, true);
	}

	public CustomViewTop(Context context, AttributeSet attrs, boolean isAbove) {
		super(context, attrs);
...
//			String message = intent.getStringExtra("message");
	...
	public void setCustomViewBehind(SlidingMenu cvb) {
	...
	private void pageScrolled(int xpos) {
		...
		int offsetPixels = 0;
		float offset = 0;
		if (widthWithMargin > 0) {
			position = xpos / widthWithMargin;
			;
			offsetPixels = xpos % widthWithMargin;
			offset = (float) offsetPixels / widthWithMargin;
		}
	...
	private void onDrawBehindFade(Canvas canvas, float openPercent) {
		...
		if (alpha > 0) {
			...
			canvas.drawRect(0, 0, getContentLeft(), getHeight(),
					mBehindFadePaint);
		}
	}
